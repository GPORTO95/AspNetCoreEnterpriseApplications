...
namespace SE.Bff.Compras.Controllers
{
    public class PedidoController : MainController
    {
        ...
        private async Task<bool> ValidarCarrinhoProdutos(CarrinhoDTO carrinho, IEnumerable<ItemProdutoDTO> produtos)
        {
            ...
            {
                var itensIndisponiveis = carrinho.Itens.Select(x => x.ProdutoId).Except(produtos.Select(p => p.Id)).ToList();

                foreach (var itemId in itensIndisponiveis)
                {
                    var itemCarrinho = carrinho.Itens.FirstOrDefault(c => c.ProdutoId == itemId);
                    AdicionarErroProcessamento($"O item {itemCarrinho.Nome} não está mais disponivel no catálogo, o remova do carrinho para prosseguir com a compra");
                }
